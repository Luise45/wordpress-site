# Wordpress Travel Blog 


---

## Projektübersicht
Dieses Projekt ist wurde erstellt um den Umgang mit dem CMS Wordpress zu üben. 
Der Blog wurde mit WordPress aufgebaut und angepasst. Wegen der kosten, ist die Seite nicht öffentlich gehostet, sondern wird nur über Screenshots gezeigt.

---

## Verwendete Technologien & Tools
Für die Erstellung der Website wurden folgende Tools und Plugins verwendet:
- **WordPress** CMS
- **Elementor** Page Builder 
- **Astra Theme** leichtes und flexibles Theme
- **Plugins**  z. B. für Formulare und zusätzliche Funktionen 
- **Forms (Kontaktformulare)** 
- **Widgets** für zusätzliche Inhalte und Layout-Elemente wie z.B. Google Maps
- **Pages & Posts** Seiten & Blog-Beiträge

---

## Aufbau der Website

### Startseite 
<img width="1920" height="1080" alt="Screenshot 2026-06-22 at 09 41 16" src="https://github.com/user-attachments/assets/161aafb2-5c6f-4e44-b871-a59ef8e9933f" />
<img width="1920" height="1080" alt="Screenshot 2026-06-22 at 09 42 05" src="https://github.com/user-attachments/assets/1dd18f95-9d66-4336-8514-0b56c15a2d59" />

### Blog-Bereich
<img width="1920" height="1080" alt="Screenshot 2026-06-22 at 09 42 18" src="https://github.com/user-attachments/assets/30ca0412-e695-486d-a0d6-0949c6e6e9bf" />

### Kontaktformular
<img width="1920" height="1080" alt="Screenshot 2026-06-22 at 09 42 52" src="https://github.com/user-attachments/assets/ba038b31-f842-46d2-afa7-b724674630aa" />

### About me 
<img width="1920" height="1080" alt="Screenshot 2026-06-22 at 09 41 54" src="https://github.com/user-attachments/assets/bc7c7434-f75a-43bc-ae6f-50c0bd98ef50" />

---

## Outcome
Während der Erstellung habe ich gelernt:
- Umgang mit WordPress als CMS
- Installation und Nutzung von Themes und Plugins
- Aufbau und Struktur einer Website
- Gestaltung von Layouts mit Elementor
- Erstellung von Formularen und Blogbeiträgen

---
## Autor
Name: Luise Tabatt
